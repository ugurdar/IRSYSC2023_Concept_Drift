# Loading libraries
packages <- c("purrr", "dplyr", "DALEX", "ggplot2")
...
calculate_distances_for_model <- function(data, window_len) {
  ...
  logit_model <- glm(y ~ ., data = windowing_data[[1]], family = "binomial")
  
  explainer_list <- map(windowing_data, function(df) {
    explainer_lr <- DALEX::explain(logit_model, data = df[, -3], y = df$y, verbose = FALSE)
...
results_list[[results_list_plot_name]] <- 
  ...
  geom_line() +
  labs(y = "PDI", x = "Batch Number", title = results_list_plot_name) +
...
ggsave("output/concept_drift_coef.png",
       plot= results_list$`PDI Results ~ Change in Coefficent`,
      )
ggsave("output/concept_drift_mean.png",
       plot= results_list$`PDI Results ~ Change in Mean of X.1 Distribution`
      )
